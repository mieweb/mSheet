...
import type { FormEngine, MatrixRow, MatrixColumn } from '@msheet/core';
 ...
 */
export function MatrixEditor({ fieldId, rows, columns, engine }: MatrixEditorProps) {
  const instanceId = useInstanceId();
  const rowsRef = React.useRef<HTMLDivElement>(null);
  const colsRef = React.useRef<HTMLDivElement>(null);

  const handleAddRow = () => {
    engine.getState().addRow(fieldId);
  ...
  const handleAddColumn = () => {
    engine.getState().addColumn(fieldId);
    requestAnimationFrame(() => {
          ...
          {rows.map((row, idx) => (
            <div key={row.id} className="row-item ms:flex ms:items-center ms:gap-1">
              ...
              <input
                ...
                type="text"
                value={row.value}
                onChange={(e) => engine.getState().updateRow(fieldId, row.id, e.currentTarget.value)}
              ...
              <button
                type="button"
                onClick={() => engine.getState().removeRow(fieldId, row.id)}
          ...
          {columns.map((col, idx) => (
            <div key={col.id} className="column-item ms:flex ms:items-center ms:gap-1">
              ...
              <input
                ...
                type="text"
                value={col.value}
                onChange={(e) => engine.getState().updateColumn(fieldId, col.id, e.currentTarget.value)}
                placeholder={`Column ${idx + 1}`}
                className="ms:flex-1 ms:min-w-0 ms:px-2 ms:py-1 ms:text-sm ms:bg-transparent ms:border ms:border-msborder ms:rounded ms:text-mstext placeholder:ms:text-mstextmuted focus:ms:outline-none focus:ms:ring-2 focus:ms:ring-msprimary focus:ms:border-msprimary"
              />
              <button
                type="button"
                onClick={() => engine.getState().removeColumn(fieldId, col.id)}
