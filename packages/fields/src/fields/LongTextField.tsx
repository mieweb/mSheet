...
export const LongTextField = React.memo(function LongTextField({
      ...
      <div className="longtext-field-preview">
        <div className="ms:grid ms:grid-cols-1 ms:gap-2 sm:ms:grid-cols-2 ms:pb-4">
          ...
          <textarea
            ...
            className="ms:px-3 ms:py-2 ms:h-24 ms:w-full ms:min-w-0 ms:border ms:border-msborder ms:bg-mssurface ms:text-mstext ms:shadow-sm ms:rounded-lg ms:max-h-60 ms:resize-y focus:ms:border-msprimary focus:ms:ring-1 focus:ms:ring-msprimary/30 ms:outline-none ms:transition-colors"
            rows={4}
          />
        </div>
      </div>
  ...
  return (
    <div className="longtext-field-edit ms:space-y-2">
      <div>
        <label
          htmlFor={`${instanceId}-longtext-question-${def.id}`}
          className="ms:block ms:text-sm ms:font-medium ms:text-mstextmuted ms:mb-1"
        >
          Question
        </label>
        <input
          id={`${instanceId}-longtext-question-${def.id}`}
          aria-label="Question"
          type="text"
          value={def.question || ''}
      ...
      <textarea
        id={`${instanceId}-longtext-preview-${def.id}`}
