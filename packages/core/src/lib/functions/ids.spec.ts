...
describe('generateFieldId', () => {
  ...
  it('should increment past existing numbered IDs', () => {
    expect(generateFieldId('text', new Set(['text', 'text-1', 'text-2']))).toBe(
      'text-3'
    );
  });

  it('should find the max number even if gaps exist', () => {
    expect(generateFieldId('text', new Set(['text', 'text-1', 'text-5']))).toBe(
      'text-6'
    );
  });
  ...
  it('should handle collisions with parentId prefix', () => {
    expect(generateFieldId('text', new Set(['s1-text']), 's1')).toBe(
      's1-text-1'
    );
  });
...
describe('generateOptionId', () => {
  ...
  it('should increment when base exists', () => {
    expect(generateOptionId(new Set(['radio-option']), 'radio')).toBe(
      'radio-option-1'
    );
  });
  ...
  it('should increment past existing numbered IDs', () => {
    expect(
      generateOptionId(
        new Set(['radio-option', 'radio-option-1', 'radio-option-2']),
        'radio'
      )
    ).toBe('radio-option-3');
  });
...
describe('generateRowId', () => {
  ...
  it('should increment when base exists', () => {
    expect(
      generateRowId(new Set(['matrix-row', 'matrix-row-1']), 'matrix')
    ).toBe('matrix-row-2');
  });
...
describe('generateColumnId', () => {
  ...
  it('should increment when base exists', () => {
    expect(
      generateColumnId(new Set(['matrix-col', 'matrix-col-1']), 'matrix')
    ).toBe('matrix-col-2');
  });
