...
const DRIVERS: FieldDefinition[] = [
  ...
  },
  {
    id: 'drv-text',
    fieldType: 'text',
    question: 'Driver Text',
    inputType: 'string',
  },
  {
    id: 'drv-number',
    fieldType: 'text',
    question: 'Driver Number',
    inputType: 'number',
  },
  {
    id: 'drv-check',
...
describe('comprehensive conditions integration', () => {
    ...
    describe('notEquals', () => {
      it('radio — true when different option selected', () => {
        const f: FieldDefinition = {
          ...
          question: 'T',
          rules: [
            fieldRule('visible', 'drv-radio', 'notEquals', 'drv-radio-a'),
          ],
        };
        const { normalized, field } = setup(f);
        expect(
      ...
      it('dropdown — true when different option', () => {
        const f: FieldDefinition = {
          ...
          question: 'T',
          rules: [
            fieldRule('visible', 'drv-dropdown', 'notEquals', 'drv-dd-m'),
          ],
        };
        const { normalized, field } = setup(f);
        expect(
    ...
    describe('includes', () => {
      it('check — option present in selection', () => {
        ...
        expect(
          resolveEffect('visible', field, normalized, {
            'drv-check': {
              selected: [sel('drv-chk-x', 'x-ray'), sel('drv-chk-y', 'yankee')],
            },
          })
        ).toBe(true);
      });
      ...
      it('multiselectdropdown — option present', () => {
        const f: FieldDefinition = {
          ...
          question: 'T',
          rules: [
            fieldRule('visible', 'drv-multiselect', 'includes', 'drv-ms-g'),
          ],
        };
        const { normalized, field } = setup(f);
        expect(
          resolveEffect('visible', field, normalized, {
            'drv-multiselect': {
              selected: [sel('drv-ms-g', 'green'), sel('drv-ms-b', 'blue')],
            },
          })
        ).toBe(true);
      });
    ...
    describe('numeric operators', () => {
      ...
      it('greaterThanOrEqual — rating value >= 3', () => {
        const f: FieldDefinition = {
          ...
          question: 'T',
          rules: [
            fieldRule('visible', 'drv-rating', 'greaterThanOrEqual', '3'),
          ],
        };
        const { normalized, field } = setup(f);
        expect(
  ...
  describe('conditional effects', () => {
    ...
    it('multiple effects — visible and required on same field with different conditions', () => {
      ...
      expect(resolveEffect('visible', field, normalized, responses2)).toBe(
        true
      );
      expect(resolveEffect('required', field, normalized, responses2)).toBe(
        false
      );
    });
  });
            ...
            conditions: [
              {
                ...
                targetId: 'drv-radio',
                operator: 'equals',
                expected: 'drv-radio-a',
              },
              {
                conditionType: 'field',
                targetId: 'drv-dropdown',
                operator: 'equals',
                expected: 'drv-dd-l',
              },
            ],
  ...
  describe('conditional sections', () => {
    ...
    it('nested condition inside a conditional section', () => {
      ...
      expect(
        resolveEffect('visible', nestedField, normalized, {
          'drv-radio': { selected: sel('drv-radio-c', 'charlie') },
  ...
  describe('every field type with expression condition', () => {
    ...
    for (const baseDef of FIELD_TYPE_DEFS) {
      it(`${baseDef.fieldType}${
        ...
        // Slider = 3 → visible
        expect(
          resolveEffect('visible', f, normalized, {
            'drv-slider': { selected: sel('drv-sl-3', '3') },
          })
        ).toBe(true);
  ...
  describe('FieldResponse shapes — condition reads correct answer from each format', () => {
    it('text answer — condition reads from answer property', () => {
      ...
      expect(
        resolveEffect('visible', target, normalized, { src: respWrong })
      ).toBe(false);
    });
    ...
    it('number text answer — condition reads numeric value from answer', () => {
      ...
      expect(
        resolveEffect('visible', target, normalized, { src: respLow })
      ).toBe(false);
    });
    ...
    it('single-select (radio) — condition reads option ID from selected', () => {
      ...
      expect(resolveEffect('visible', target, normalized, { src: respB })).toBe(
        false
      );
    });
    ...
    it('multi-select (check) — condition reads from selected array via includes', () => {
      ...
      const respMissing: FieldResponse = {
        selected: [{ id: 'c-1', value: 'A' }],
      };
      expect(
        resolveEffect('visible', target, normalized, { src: respMissing })
      ).toBe(false);
    });
    ...
    it('undefined response → empty is true, notEmpty is false', () => {
      ...
      expect(
        resolveEffect('visible', targetEmpty, normalized1, {
          src: resp as unknown as FieldResponse,
        })
      ).toBe(true);
      expect(resolveEffect('visible', targetNotEmpty, normalized2, {})).toBe(
        false
      );
    });
    ...
    it('expression reads .length on text answer', () => {
      ...
      expect(
        resolveEffect('visible', target, normalized, { src: respShort })
      ).toBe(false);
    });
